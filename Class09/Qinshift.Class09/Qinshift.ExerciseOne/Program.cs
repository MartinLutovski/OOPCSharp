using Qinshift.ExerciseOne.Models;
...
try
{
    ...
    {
        throw new Exception("Invalid input! You have to enter '1' or '2'.");
    }
    Console.WriteLine("Movies in the cinema:");
    Console.WriteLine("1.Show all movies \n2.2.Show by genre");
    int movieInput = int.Parse(Console.ReadLine());
    if (movieInput == 1) {
        Console.WriteLine("Chose a movie to watch: ( write the name )");
        foreach (var movie in currentCinema.Movies)
        {
            Console.WriteLine(movie.Title);
        }
        string movieNameInput = Console.ReadLine();
        currentCinema.MoviePlaying(currentCinema.Movies.Where(x => x.Title.ToLower().Trim() == movieNameInput.ToLower().Trim()).FirstOrDefault());
