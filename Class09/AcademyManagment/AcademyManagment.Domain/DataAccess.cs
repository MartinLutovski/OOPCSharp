...
namespace AcademyManagment.Domain
{
    public class DataAccess
    {
        ...
        {
            Admin adminFromDb = _database.Admins.Find(x => x.Username.ToLower().Trim() == username.ToLower().Trim() && x.Password == password);
            return adminFromDb;
        }

        public Trainer GetTrainer(string username, string password)
        {
            Trainer trainerFromDb = _database.Trainers.Find(x => x.Username.ToLower().Trim() == username.ToLower().Trim() && x.Password == password);
            return trainerFromDb;
        }

        public Student GetStudent(string username, string password)
        {
            Student studentFromDb = _database.Students.Find(x => x.Username.ToLower().Trim() == username.ToLower().Trim() && x.Password == password);
